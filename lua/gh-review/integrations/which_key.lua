--- Which-key integration: register <leader>gp group
local M = {}

local config = require("gh-review.config")

--- Register keybindings with which-key
function M.register()
  local ok, wk = pcall(require, "which-key")
  if not ok then
    return
  end

  local prefix = config.get().keymaps.prefix
  wk.add({
    { prefix, group = "PR Review" },
    { prefix .. "o", desc = "Checkout PR" },
    { prefix .. "O", desc = "Review PR for current branch" },
    { prefix .. "f", desc = "Open/close file tree" },
    { prefix .. "F", desc = "Focus file tree" },
    { prefix .. "c", desc = "Open/close comments panel" },
    { prefix .. "Y", desc = "Focus comments panel" },
    { prefix .. "r", desc = "Reply to thread" },
    { prefix .. "n", desc = "New comment thread (pending review)" },
    { prefix .. "N", desc = "New comment (posted immediately)" },
    { prefix .. "p", desc = "Review/submit unsubmitted comments" },
    { prefix .. "a", desc = "Approve / comment / request changes on the PR" },
    { prefix .. "s", desc = "Next commit in the jj stack" },
    { prefix .. "S", desc = "Previous commit in the jj stack" },
    { prefix .. "k", desc = "Open/close the jj stack picker" },
    { prefix .. "t", desc = "Toggle resolve" },
    { prefix .. "v", desc = "View comment at cursor" },
    { prefix .. "d", desc = "PR description" },
    { prefix .. "D", desc = "Toggle diff overlay" },
    { prefix .. "e", desc = "Open file with diff overlay" },
    { prefix .. "w", desc = "Open diffview.nvim" },
    { prefix .. "u", desc = "Open unified single-buffer diff view" },
    { prefix .. "W", desc = "Toggle ignore whitespace-only changes" },
    { prefix .. "L", desc = "Toggle linematch (char vs grouped)" },
    { prefix .. "m", desc = "Toggle reviewed mark on current file" },
    { prefix .. "V", desc = "Toggle split / inline view" },
    { prefix .. "R", desc = "Refresh PR data" },
    { prefix .. "q", desc = "Close review" },
    { "]d", desc = "Next diff hunk (cross-file)" },
    { "[d", desc = "Prev diff hunk (cross-file)" },
    { "<leader>gN", desc = "Next PR file" },
    { "<leader>gP", desc = "Prev PR file" },
  })
end

return M
